...
app.config["SMTP_PASSWORD"] = os.getenv("SMTP_PASSWORD", "yxkm ucqq sqry vlgi")
...
MAX_GALLERY_IMAGES = int(os.getenv("MAX_GALLERY_IMAGES", "4"))
...
def category_gallery(category_slug):
    ...
    images = _list_images(folder)
    display_images = images[:MAX_GALLERY_IMAGES]
    image_urls = [
        f"{COLLECTION_DIR}/{selected['slug']}/{img.name}" for img in display_images
    ]
